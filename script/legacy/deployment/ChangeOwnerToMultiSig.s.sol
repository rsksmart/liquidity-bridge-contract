 ...
 *
 * Usage:
 *   forge script script/ChangeOwnerToMultiSig.s.sol:ChangeOwnerToMultiSig --rpc-url <RPC_URL> --broadcast --verify
 *
...
contract ChangeOwnerToMultiSig is Script {
    ...
    function run() external {
        ...
        require(
            multisigConfig.multisigAddress != address(0),
            "Multisig address not found for this network"
        );
        require(
            multisigConfig.expectedOwners.length > 0,
            "No expected owners configured"
        );
    ...
    function getMultisigConfig() internal view returns (MultisigConfig memory) {
        ...
        if (chainId == 30) {
            ...
            owners[0] = vm.envOr(
                "MULTISIG_OWNER_1_MAINNET",
                address(0xd50C4c5577229C81b19FE1c7D413c2D79CcF7901)
            );
            owners[1] = vm.envOr(
                "MULTISIG_OWNER_2_MAINNET",
                address(0x15105d3E5F7752F0F7ea0e7B1F3B113701080355)
            );
            owners[2] = vm.envOr(
                "MULTISIG_OWNER_3_MAINNET",
                address(0xA420aF120Ec6515870B65e811Fa7cE147d491402)
            );
            owners[3] = vm.envOr(
                "MULTISIG_OWNER_4_MAINNET",
                address(0xA3FD0eeDCA5Ba7504e19B7e0660F21d25FEFE77f)
            );

            return
                MultisigConfig({
                    multisigAddress: vm.envOr(
            ...
            owners[0] = vm.envOr(
                "MULTISIG_OWNER_1_TESTNET",
                address(0x8E925445BdA88C9F980976dB098Cb1c450BFc719)
            );
            owners[1] = vm.envOr(
                "MULTISIG_OWNER_2_TESTNET",
                address(0xF18eD4047ee49D26034AEA5Dafdb0e1895E8DDc1)
            );
            owners[2] = vm.envOr(
                "MULTISIG_OWNER_3_TESTNET",
                address(0xbd98f2cA40b4Ab54161aC29120251e7d0A4C666E)
            );
            owners[3] = vm.envOr(
                "MULTISIG_OWNER_4_TESTNET",
                address(0x892813507Bf3aBF2890759d2135Ec34f4909Fea5)
            );
            owners[4] = vm.envOr(
                "MULTISIG_OWNER_5_TESTNET",
                address(0x1e0a10395E9DeBD5C109bd1116fFfAa3F4a543B2)
            );
    ...
    function _validateMultisig(
        ...
        try IGnosisSafe(multisigAddress).getOwners() returns (
            ...
            for (uint256 i = 0; i < expectedOwners.length; i++) {
                ...
                require(
                    ...
                    string(
                        abi.encodePacked(
                            "Expected owner not found: ",
                            vm.toString(expectedOwners[i])
                        )
                    )
                );
            }
    ...
    function _transferContractOwnership(
        ...
        address currentDeployer = vm.addr(
            vm.envUint(
                block.chainid == 30
        ...
        console.log(
            "Contract ownership transferred successfully to:",
            newOwner
        );
    }
    ...
    function _transferProxyAdminOwnership(
        ...
        if (currentAdminOwner != currentDeployer) {
            console.log(
                "WARNING: Current admin owner is different from deployer!"
            );
            console.log("Admin owner:", currentAdminOwner);
            console.log("Deployer:", currentDeployer);
            console.log(
                "You need to run this script with the admin owner's private key, or transfer admin ownership first."
            );
        ...
        require(
            verifyAdminOwner == newOwner,
            "Proxy admin ownership transfer failed"
        );

        console.log(
            "Proxy admin ownership transferred successfully to:",
            newOwner
        );
    }
    ...
    function runWithCustomMultisig(address customMultisigAddress) external {
        ...
        require(proxyAddress != address(0), "Proxy address must be provided");
        require(
            customMultisigAddress != address(0),
            "Custom multisig address cannot be zero"
        );
