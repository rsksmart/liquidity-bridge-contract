...
- **Static address generation:** the deposit address is derived deterministically from the
  user's RSK address (`getPegInAddress`), stable across calls and re-deploys.
...
| 07 | `07-resolve-pegin.sh` | `resolvePegIn` — LP reimbursement (currently blocked by finding B) |
...
- **B (open, EB epic):** `resolvePegIn` returns `-900` — the registry's address derivation differs
  from the native fast-bridge's, so the bridge can't settle. Step 07 documents this.
