...
interface IFlyoverConfigurations {
    ...
    /// @dev Admin-only. Values are validated against the active bounds at queue time.
    ...
    /// admin change
    /// @dev Admin-only. Reverts before the delay elapses. Values are re-validated against the
    /// active bounds, which may have moved during the delay.
