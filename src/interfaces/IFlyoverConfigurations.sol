...
interface IFlyoverConfigurations {
    ...
    /// @dev Struct fields are ABI and freeze here. Mirrors peg-in fee/limits/tiers, plus
    /// claim / fulfillment deadlines, LP BTC fee reimbursement, deposit confirmations,
    /// and the short-delivery miner-fee cap.
    ...
    /// @param gasFee BTC tx fee reimbursement paid to the LP, in wei (snapshotted onto the quote)
    /// @param depositConfirmations BTC confirmations snapshotted as quote.depositConfirmations
    ...
    /// @dev `fixedFee + percentageFee·amount / 10_000`.
