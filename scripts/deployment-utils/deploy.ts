...
type deployFunction = (name: string, network: string, act: () => Promise<string>) => Promise<DeployedContractInfo>;
...
export const read: () => DeploymentConfig = () => Object.keys(testConfig[UNIT_TEST_NETWORK]).length > 0 ? testConfig : JSON.parse(readFileSync(ADDRESSES_FILE).toString());
const write = (newConfig: DeploymentConfig) => {
    const oldConfig = read();
    writeFileSync(ADDRESSES_FILE, JSON.stringify({ ...oldConfig, ...newConfig }, null, 2));
};


export const deploy:deployFunction = async (name: string , network: string, act: () => Promise<string>) => {
    const oldConfig = read();
    ...
    if (!oldConfig[network][name].deployed || network === LOCAL_NETWORK) {
        ...
        console.warn(`${name} has already be deployed [address: ${oldConfig[network][name].address}]. If you want to deploy it, please set deployed attribute to false on addresses.json file.`)
    }
