...
contract("LiquidityProviderContract", async () => {
    ...
    it("should protect setProviderStatus from external calls", async () => {
        await instance.setProviderStatus("0x0000000000000000000000000000000000000000", true).catch(assertAccessControlBlock);
    });
    it("should protect register from external calls", async () => {
        await instance.register("", 1, 1, 1, 1, "", true, "").catch(assertAccessControlBlock);
    });
    it("should protect deposit from external calls", async () => {
        await instance.deposit().catch(assertAccessControlBlock);
    });
    it("should protect addCollateral from external calls", async () => {
        await instance.addCollateral().catch(assertAccessControlBlock);
    });
    it("should protect addPegoutCollateral from external calls", async () => {
        await instance.addPegoutCollateral().catch(assertAccessControlBlock);
    });
    it("should protect withdraw from external calls", async () => {
        await instance.withdraw(1).catch(assertAccessControlBlock);
    });
    it("should protect withdrawCollateral from external calls", async () => {
        await instance.withdrawCollateral().catch(assertAccessControlBlock);
    });
    it("should protect withdrawPegoutCollateral from external calls", async () => {
        await instance.withdrawPegoutCollateral().catch(assertAccessControlBlock);
    });
    it("should protect resign from external calls", async () => {
        await instance.resign().catch(assertAccessControlBlock);
    });
...
contract("PeginContract", async () => {
    ...
    it("should protect callForUser from external calls", async () => {
        await instance.callForUser(utils.asArray(quote)).catch(assertAccessControlBlock);
    });
    it("should protect registerPegIn from external calls", async () => {
        await instance.registerPegIn(utils.asArray(quote), [], [], [], 1).catch(assertAccessControlBlock);
    });
...
contract("PegoutContract", async () => {
    ...
    it("should protect refundPegout from external calls", async () => {
        await instance.refundPegOut("0x00", [], "0x00", 1, []).catch(assertAccessControlBlock);
    });
    it("should protect depositPegout from external calls", async () => {
        await instance.depositPegout(quote, []).catch(assertAccessControlBlock);
    });
