...
abstract contract PegInRegistryTestBase is Test {
    ...
    function _programProof(bytes memory txBytes, bytes32 blockHash, uint256 path, bytes32[] memory hashes) internal {
        bridge.setExpectedProof(BtcUtils.hashBtcTx(txBytes), blockHash, path, hashes);
    }
    ...
    function _register(address rskAddr, uint64 value, address caller) internal {
        ...
        vm.prank(caller);
        registry.registerAddress(rskAddr, txBytes, BLOCK_HASH, MERKLE_PATH, hashes);
    }
