...
contract PegInAddressRegistryRegisterTest is PegInRegistryTestBase {
    ...
    function _depositPkScript(address rskAddr) internal view returns (bytes memory) {
    ...
    function _buildDepositTx(bytes memory pkScript, uint64 value) internal pure returns (bytes memory) {
    ...
    function _register(address rskAddr, uint64 value, address caller) internal {
        bytes memory txBytes = _buildDepositTx(_depositPkScript(rskAddr), value);
        vm.prank(caller);
        registry.registerAddress(rskAddr, txBytes, BLOCK_HASH, MERKLE_PATH, new bytes32[](0));
    }

    function _foldRoot(bytes32 prevRoot, address rskAddr) internal pure returns (bytes32) {
    ...
    function test_revert_when_already_registered() public {
        ...
        registry.registerAddress(FIXTURE_RSK, garbageTx, BLOCK_HASH, MERKLE_PATH, new bytes32[](0));
    }
    ...
    function test_revert_when_no_matching_output() public {
        ...
        registry.registerAddress(FIXTURE_RSK, txBytes, BLOCK_HASH, MERKLE_PATH, new bytes32[](0));
    }
    ...
    function test_confirmed_tx_wrong_address_still_reverts() public {
        ...
        registry.registerAddress(OTHER_RSK, txBytes, BLOCK_HASH, MERKLE_PATH, new bytes32[](0));
    }
    ...
    function test_address_rederived_not_trusted() public {
        ...
        registry.registerAddress(attackerPresented, txBytes, BLOCK_HASH, MERKLE_PATH, new bytes32[](0));
    }
