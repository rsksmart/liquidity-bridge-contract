...
contract CollateralInvariantTest is Test {
    ...
    function setUp() public {
        ...
        bytes memory initData = abi.encodeCall(
            ...
            (owner, 30, MIN_COLLATERAL, RESIGN_DELAY, REWARD_PERCENTAGE)
        );
    ...
    function resignAndWithdraw(uint256 providerSeed) external {
        ...
        // Resign first
        vm.prank(provider);
        try collateralManagement.resign() {} catch { return; }
    ...
    function invariant_NoNegativeCollateral() public view {
        for (uint256 i = 0; i < providers.length; i++) {
            uint256 pegInCollateral = collateralManagement.getPegInCollateral(providers[i]);
            uint256 pegOutCollateral = collateralManagement.getPegOutCollateral(providers[i]);
    ...
    function _getOrCreateProvider(uint256 seed) internal returns (address provider) {
        if (providers.length > 0 && seed % 3 != 0) {
            return providers[seed % providers.length];
        }
        
        provider = address(uint160(uint256(keccak256(abi.encode(seed, providers.length)))));
